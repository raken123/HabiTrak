...
export function realtouchInpaintPrompt({ scene, userHint }) {
  return [
    ...
    '',
    // In Eco Mode there is no lookup, so there is nothing to follow: say so
    // rather than leaving an empty section the model has to interpret.
    ...(scene ? [
      ...
      'Nothing was looked up about this place. Work only from the photograph:',
      'continue what runs into the region and do not invent a landmark, a sign,',
      'or any specific thing you cannot see the rest of.',
      '',
    ]),
    'Requirements:',
...
export function miniRemovePrompt({ target, scene = '' }) {
  return [
    ...
    ...(scene ? [
      ...
      'Nothing was looked up about this place. Work only from the photograph and',
      'do not invent a landmark, a sign or anything specific you cannot see the',
      'rest of.',
      '',
    ]),
